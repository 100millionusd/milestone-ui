...
function hasSafeMarkerMs(m: any): boolean {
  ...
  // SAFE in-flight = pre-exec states ONLY
...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    ...
    // 2) Resume polling for still-pending items
    try {
  ...
  async function pollUntilPaid(
    ...
    for (let i = 0; i < tries; i++) {
      try {
        ...
        if (m && hasSafeMarkerMs(m)) {
          // still in-flight (queued/submitted/executing) → keep local pending
          await refreshApproved(bidId);
          await refreshProofs();
          try {
            try { (await import('@/lib/api')).invalidateBidsCache?.(); } catch {}
    ...
    // final: do NOT auto-clear; wait for paid markers
    try {
      const bid = await getBid(bidId);
      const m = bid?.milestones?.[milestoneIndex];
      if (m && isPaidMs(m)) {
  ...
  async function handleReleasePayment(idx: number) {
    ...
    try {
      ...
      // queued → hide buttons now + broadcast + poll
