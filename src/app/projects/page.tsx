...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [proposalsData, bidsData] = await Promise.all([getProposals(), getBids()]);
        // ✅ keep ALL proposals (don’t filter out archived)
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="space-y-6">
        {archivedProjects.map((project) => (
          <div
            key={project.proposalId}
            className="border rounded-lg p-6 bg-amber-50 hover:shadow-sm transition"
          >
            <div className="flex items-center gap-2 mb-1">
              <h2 className="font-semibold text-xl">{project.title}</h2>
