...
type ProofRow = {
  ...
  // ADD THIS: milestone amount from proof data
  amount?: number | string | null;        // milestone amount
};
...
function normalizeProofs(rows: any[]): ProofRow[] {
  return (rows || []).map((r: any, index) => {
    ...
    // ADD THIS: Extract the milestone amount
...
// CORRECTED: Create normal payments from proofs - USE PROOF AMOUNT
  ...
  proofs.forEach((proof, index) => {
    ...
    if (proof.status === 'paid' && !proof.safe_payment_tx_hash && !proof.safe_tx_hash) {
      ...
      console.log(`Creating COMPLETED normal payment for proof ${proof.id}`, {
        ...
        amount: proof.amount // This should come from the proof data
      });
      ...
      normalPayments.push({
        ...
        amount_usd: proof.amount || null, // Use the proof amount
        status: 'completed',
