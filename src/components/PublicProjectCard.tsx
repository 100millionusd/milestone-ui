    ...
    return {
      ...
      details:
        Array.isArray(a.changedFields) && a.changedFields.length
          ...
          : undefined,
      ipfs: ipfs ? toGatewayUrl(ipfs, { gateway: 'https://cloudflare-ipfs.com' }) : undefined,
...
export default function PublicProjectCard({ project }: { project: Project }) {
  ...
  const ipfsHref = cid ? toGatewayUrl(cid, { gateway: 'https://cloudflare-ipfs.com' }) : undefined;
  const explorerHref =
                          ...
                          >
                            {isImg ? (
                              <div className="relative w-full aspect-video">
                                <Image
                                  src={toGatewayUrl(String(f.url), { width: 400, height: 300, fit: 'cover', format: 'webp', gateway: 'https://cloudflare-ipfs.com' })}
                                  alt={f.name || `file ${i + 1}`}
                                  fill
    ...
    <>
      <div className="rounded-2xl border border-gray-200 overflow-hidden bg-white shadow-sm">
        ...
        >
          {project.coverImage ? (
            <Image
              src={toGatewayUrl(project.coverImage, { width: 1200, format: 'webp', gateway: 'https://cloudflare-ipfs.com' })}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {project.images.slice(1, 10).map((u: string, i: number) => (
                        ...
                        >
                          <Image
                            src={toGatewayUrl(u, { width: 400, height: 300, fit: 'cover', format: 'webp', gateway: 'https://cloudflare-ipfs.com' })}
                            alt={`image ${i + 1}`}
                            fill
                            sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 33vw"
                            style={{ objectFit: 'cover' }}
      ...
      {lightboxUrl && (
        ...
        >
          ...
          >
            <Image
              src={toGatewayUrl(lightboxUrl, { gateway: 'https://cloudflare-ipfs.com' })}
              alt="Zoomed image"
              fill
              sizes="100vw"
