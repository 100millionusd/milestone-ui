...
export default function ChangeRequestsPanel(props: Props) {
  ...
  // FIXED: Removed 'idx' from dependencies to prevent loop.
  ...
  }, [proposalId, forceMilestoneIndex]); // Removed idx

  // Trigger load only on proposalId change (and forceMilestoneIndex), NOT activeMilestoneIndex
  useEffect(() => {
    load();
  }, [load, proposalId]); // Removed idx

  // Separate Effect: Auto-focus logic (Runs only when data arrives)
  useEffect(() => {
    if (typeof forceMilestoneIndex === "number") return;
    if (!rows.length) return;

    const present = new Set<number>(
      rows.map((row) => Number(row.milestoneIndex)).filter((x) => Number.isFinite(x))
    );
    // Only switch if current index has no data, but another index does
    if (!present.has(activeMilestoneIndex) && present.size > 0) {
      const latest = Math.max(...Array.from(present.values()));
      if (latest !== activeMilestoneIndex) setActiveMilestoneIndex(latest);
    }
  // We intentionally exclude activeMilestoneIndex from deps to avoid flip-flop loops
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rows, forceMilestoneIndex]); 
  ...
  // Logic for Action Required (Open + Not Resolved + No Reply)
